...
public class MLAgent : MonoBehaviour
{
    ...
    public PerceptionBase.ACTION_TYPE AgentInput()
    {
        ...
        {
            case ModelType.MLP:
                // 1) Leer parámetros actuales de la percepción
                //    PlayerPerception.Parameters es un MLGym.Parameters
                MLGym.Parameters p = perception.Parameters;
                float[] input = p.ConvertToFloatArray();

                // 2) Ejecutar el modelo (incluye scaler, OHE, etc.)
                float[] outputs = RunFeedForward(input);

                // 3) Obtener la acción con mayor probabilidad
                action = this.mlpModel.Predict(outputs);

                // 4) Guardar la decisión para análisis
                recorder.AIRecord(action);
                break;
        }

        PerceptionBase.ACTION_TYPE inputEnum = Record.ConvertLabelToInput(action);
        return inputEnum;
    }
    ...
    {
        // 1) Eliminar columnas que no usamos en el modelo (si procede)
        modelInput = modelInput
            .Where((value, index) => !indicesToRemove.Contains(index))
            .ToArray();

        // 2) Aplicar StandarScaler (mismas medias y desviaciones que en sklearn)
        float[] scaled = standarScaler.Transform(modelInput);

        // 3) Aplicar OneHotEncoding (si lo has configurado en oHE_Elements)
        float[] finalInput = oneHotEncoding.Transform(scaled);

        // 4) Guardar el input transformado para poder reproducir en Python
        recorder.AIRecord(finalInput);

        // 5) Ejecutar la red neuronal MLP
        float[] outputs = this.mlpModel.FeedForward(finalInput);
